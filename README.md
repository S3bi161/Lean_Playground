...
This is a formalization of *Dependent Assertion Logic* (DA) in Lean, heavily based on "A semantic foundation for dependent assertion reasoning" by Dr. Lukas Grätz, 2026, unpublished; this paper is referenced in the following and in comments as "paper". 
...
## Design Differences
Even though the formalization tries to mirror the paper very literally (which is why the axiomatic approach for REPEAT_arr semantics is chosen instead of inventing an operational semantics or similar), some things are conceptually different, e.g. to accomodate for Lean notation quirks or the formal type theory.
...
You may read ∘ᵢ as dynamic index concatenation and ∘ᵣ as relation concatenation. The index ₗ on logical connectives is to distinguish it from Leans connectives (l standing for logic, since d or f have no unicode subscripts).
...
It also means that the concatenation of dynamic indices ∘ᵢ and the concatenation of relations ∘ᵣ are strictly distinguished other than in the paper.
